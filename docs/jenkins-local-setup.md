...
В текущем `Jenkinsfile` параметр `SELENOID_URL` больше не объявлен в `parameters`,
поэтому при ручном запуске job Jenkins не запрашивает его ввод.

`Jenkinsfile` всегда пробует автоопределение URL в таком порядке:
